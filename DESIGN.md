...
아래 항목은 기능을 추가할 때 먼저 테스트해야 하는 위험 구간이다.

1. **VBA 문자열 처리**
   - 문자열 안의 따옴표(`""`)가 깨지면 결과 코드가 바로 문법 오류가 된다.
   - 예: `MsgBox "파일명은 ""report.xlsx"" 입니다"` 같은 케이스.
   - 확인 방법: 문자열을 수정한 뒤 원본 따옴표 개수와 이스케이프가 유지되는지 비교.

2. **라인 이어쓰기**
   - VBA는 줄 끝의 `_`로 다음 줄을 이어 쓸 수 있다.
   - 현재 토크나이저가 논리 라인을 합쳐 다루므로 오프셋 계산이 틀어지면 다른 값을 바꿀 수 있다.
   - 확인 방법: `Range("A1", _`처럼 여러 줄로 나뉜 함수 호출 샘플을 추가.

3. **주석과 코드 구분**
   - `'` 뒤의 텍스트는 코드가 아니므로 위젯으로 추출되면 안 된다.
   - 문자열 안의 `'`는 주석이 아니므로 그대로 보존되어야 한다.
   - 확인 방법: `"it's ok"`와 `' Range("A1")`가 섞인 샘플을 추가.

4. **동일한 값이 여러 번 나오는 경우**
   - 같은 `"매출"`이나 `100`이 여러 곳에 있을 때 하나만 바꾸는지, 묶어서 바꾸는지 정책이 필요하다.
   - 현재는 위치별로 별도 위젯이 생기는 방향이 안전하다.
   - 추후 "같은 값 한 번에 변경" 기능을 넣을 때는 사용자가 명시적으로 선택하게 해야 한다.

5. **숫자 0, 1, -1의 의미**
   - VBA에서 `0`, `1`, `-1`은 단순 설정값이 아니라 인덱스, True 상수, 옵션 플래그일 수 있다.
   - 잘못 노출하면 매크로 구조가 깨질 수 있으므로 기본적으로 숨기는 정책을 유지한다.

6. **Excel 객체 이름 불일치**
   - 시트명, 피벗 필드명, 차트명, 슬라이서명이 실제 파일과 다르면 VBA 실행 시 오류가 난다.
   - UI에서는 "실제 이름과 정확히 맞아야 함"을 계속 안내해야 한다.

7. **직접 편집 후 stale offset**
   - 사용자가 왼쪽 코드를 직접 바꾸면 기존 오프셋이 더 이상 맞지 않는다.
   - 직접 입력 후에는 반드시 재분석하고, 재분석 전 변경 위젯은 비활성화하는 것이 안전하다.

8. **복사 권한 실패**
   - 브라우저나 `file://` 환경에 따라 클립보드 API가 실패할 수 있다.
   - 현재처럼 `textarea.select()`와 `document.execCommand("copy")` fallback을 유지한다.

9. **한글/특수문자 인코딩**
   - 붙여넣기 기반은 비교적 안전하지만, 파일 업로드 기능을 추가하면 CP949/UTF-8 감지가 필요하다.
   - 파일 입력을 넣기 전까지는 저장 파일 업로드를 핵심 기능으로 넣지 않는다.

10. **너무 공격적인 휴리스틱**
    - "인식 많이 하기"보다 "잘못 인식하지 않기"가 중요하다.
    - 모르는 함수의 리터럴은 기본 숨김으로 두고, `@param` 주석으로만 노출시키는 정책을 유지한다.

## 추가 기능 아이디어

아래 순서대로 넣으면 현재 앱의 목적을 해치지 않으면서 완성도를 높일 수 있다.

### 바로 넣기 좋은 기능

1. **샘플 불러오기 버튼**
   - `samples/`의 예제 매크로를 버튼으로 불러와 사용자가 바로 테스트할 수 있게 한다.
   - 처음 접속한 사람이 "무엇을 붙여넣어야 하는지" 바로 이해할 수 있다.

2. **변경 전/후 비교 보기**
   - 사용자가 바꾼 값만 별도 목록으로 보여준다.
   - 복사 전에 "내가 무엇을 바꿨는지" 확인할 수 있어 실수를 줄인다.

3. **실행 전 체크리스트**
   - 시트명, 파일 경로, 범위 주소, 피벗 필드명처럼 오류가 자주 나는 값을 모아 보여준다.
   - 예: "이 시트 이름이 실제 엑셀 파일에 있나요?"

4. **위험 작업 경고**
   - `.Delete`, `.Clear`, `.Save`, `.Close SaveChanges:=True`, `.Kill` 같은 코드가 있으면 요약 영역에 경고를 표시한다.
   - 비개발자가 실행 전에 데이터 삭제/저장 위험을 인지할 수 있다.

5. **파일 경로 도우미**
   - Windows 경로의 `\` 처리, 공백 포함 경로, 상대 경로를 안내한다.
   - 예: `C:\Users\...\Desktop\report.xlsx` 형식인지 확인.

6. **셀 범위 검증**
   - `"A1:B10"`, `"A:H"`, `"Sheet1!A1:B10"` 같은 형식인지 간단히 검사한다.
   - 틀린 형식이면 입력칸 아래에 경고를 띄운다.

7. **변경 히스토리**
   - 마지막 5~10개 변경을 되돌릴 수 있게 한다.
   - 현재 `모두 초기화`보다 세밀한 복구가 가능하다.

### 중간 난이도 기능

1. **같은 값 묶어서 변경**
   - 같은 시트명이나 파일명이 여러 번 등장하면 "같은 값 모두 변경" 토글을 제공한다.
   - 기본은 꺼두고 사용자가 켤 때만 적용한다.

2. **위젯 검색/필터**
   - 큰 매크로에서 "시트", "파일", "피벗" 같은 키워드로 설정값을 빠르게 찾게 한다.

3. **Sub/Function별 접기**
   - 매크로가 여러 개인 경우 요약과 설정값을 Sub 단위로 묶어 접고 펼친다.

4. **자동 저장**
   - 브라우저 `localStorage`에 마지막 코드와 변경 상태를 저장한다.
   - 새로고침해도 작업을 잃지 않게 한다.

5. **어노테이션 생성 도우미**
   - 인식되지 않은 값 옆에 "이 값을 설정값으로 표시하는 주석 만들기"를 제공한다.
   - 결과 코드에 `' @param ...` 주석을 자동 삽입할 수 있다.

6. **샘플 기반 테스트 페이지**
   - `samples/*.bas`를 모두 돌려 인식 개수와 주요 라벨이 기대값과 맞는지 확인한다.
   - 정적 HTML 프로젝트라도 브라우저 테스트 안정성을 높일 수 있다.

### 나중에 검토할 기능

1. **AI 요약/설명 연결**
   - 사용자가 원하면 OpenAI API 키를 넣어 더 자연스러운 코드 설명을 생성한다.
   - 기본 앱은 계속 100% 로컬로 유지하고, AI 기능은 선택 사항으로 둔다.

2. **VBA 문법 검사**
   - 브라우저에서 완전한 VBA 실행 검증은 어렵지만, 괄호/따옴표/`Sub`-`End Sub` 균형은 검사할 수 있다.

3. **엑셀 파일 업로드 기반 검증**
   - 시트명과 범위가 실제 파일에 있는지 확인한다.
   - 구현 범위가 커지고 개인정보 이슈가 생기므로 MVP 이후에만 고려한다.

4. **템플릿 저장소**
   - 자주 쓰는 매크로를 "파일 합치기", "피벗 생성", "보고서 PDF 저장" 같은 템플릿으로 제공한다.
   - 템플릿마다 안전한 설정값에 `@param`을 미리 붙여 둔다.

## 다음 개발 우선순위 제안

1. 샘플 불러오기 버튼을 추가한다.
2. 위험 작업 경고를 요약 영역에 추가한다.
3. 셀 범위와 파일 경로 입력값에 간단한 검증 메시지를 붙인다.
4. 변경 전/후 비교 패널을 추가한다.
5. 샘플 기반 회귀 테스트를 만든다.
